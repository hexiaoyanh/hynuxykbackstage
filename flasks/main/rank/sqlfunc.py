...
def create_table_is_not_exists(table_name):
    sql = "create table if not exists `grade_{}`(`id` int not null auto_increment primary key,\
    `userid` varchar(64) not null,\
    `bz` varchar(64),\
    `cjbsmc` varchar(64),\
    `kclbmc` varchar(64),\
    `zcj` varchar(64),`xm` varchar(64),\
    `xqmc` varchar(64),\
    `kcxzmc` varchar(64),`ksxzmc` varchar(64),\
    `kcmc` varchar(64),\
    `xf` varchar(10),\
...
def insert_data(table_name, data):
    sql = "insert into grade_{} (userid, bz, cjbsmc, kclbmc, zcj, xm, xqmc, kcxzmc, ksxzmc, kcmc, xf, bj) value (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)".format(
        table_name)
    value = (
        data['userid'], data['bz'], data['cjbsmc'], data['kclbmc'], data['zcj'], data['xm'], data['xqmc'],
        data['kcxzmc'],
        data['ksxzmc'],
        data['kcmc'], data['xf'], data['bj'])
